...
class RestAPI {
    ...
    const DEFAULT_SALT = 'REPLACE_THIS_WITH_A_SALT';
    ...
    */
    ...
    private $general_errors = array(
        ...
        24 => 'The API salt has not been changed, make sure you use another salt before using this API',
    );
    ...
    private $xenAPI, $method, $data = array(), $hash = FALSE, $apikey = FALSE, $salt = NULL;
    ...
    */
    ...
    public function isDefaultSalt() {
        return $this->getSalt() == self::DEFAULT_SALT;
    }
    ...
    public function processRequest() {
        ...
        switch (strtolower($this->getAction())) {
            case 'authenticate':
                ...
                */
                if ($this->hasRequest('login_hash')) {
                    if (!$this->getRequest('login_hash')) {
                        // Throw error if the 'login_hash' argument is set but empty.
                        $this->throwError(1, 'login_hash');
                        break;
                    }
                    if (!$this->hasSalt() || $this->isDefaultSalt()) {
                        // API salt is not set or not changed from the default API salt.
                        $this->throwError(24);
                    }
                    $user_information = json_decode(base64_decode($this->decryptString($this->getRequest('login_hash'))), TRUE);
                    if ($user_information === NULL) {
                        $this->throwError(5, 'Unable to decrypt login_hash');
                    } else if (!array_key_exists('username', $user_information)) {
                        $this->throwError(3, 'username');
                        break;
                    } else if (!array_key_exists('password', $user_information)) {
                ...
                if (!$user->isRegistered()) {
                    ...
                    $this->throwError(4, 'user', $user_information['username']);
                } else {
                    // Requested user was registered, check authentication.
                    if ($user->validateAuthentication($user_information['password'])) {
        ...
        case 'createprofilepost':
            ...
            case 'login':
                ...
                *
                * EXAMPLE:
                *   - api.php?action=login&login_hash=HASHED_AND_SALTED_USERNAME_AND_PASSWORD
                */
                if (!$this->hasRequest('login_hash')) {
                    // The 'login_hash' argument has not been set, throw error.
                    $this->throwError(3, 'login_hash');
                    break;
                } else if (!$this->getRequest('login_hash')) {
                    // Throw error if the 'login_hash' argument is set but empty.
                    $this->throwError(1, 'login_hash');
                    break;
                }
                ...
                if ($user_information === NULL) {
                    ...
                    $this->throwError(3, 'password');
                    break;
                } else if (!array_key_exists('redirect_url', $user_information)) {
                    $this->throwError(3, 'redirect_url');
                    break;
                }
                ...
                if (!$user->isRegistered()) {
                    ...
                    if (!$user->validateAuthentication($user_information['password'])) {
                        ...
                        die(header('Location: ' . $user_information['redirect_url']));
                    }
                }
                break;
            case 'register':
...
class XenAPI {
    ...
    */
    ...
    public function login($user_id, $username) {
        ...
        $session = XenForo_Session::getPublicSession($request);
